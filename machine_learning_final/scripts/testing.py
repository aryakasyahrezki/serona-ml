import cv2
import mediapipe as mp
import joblib
import numpy as np
import time
import os
import math

# ==========================================
# 1. SETUP & LOAD MODEL
# ==========================================
script_dir = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(script_dir, '../models/final_model.pkl')

print(f"📂 Model Path: {MODEL_PATH}")
print("⏳ Loading AI Models...")

try:
    artifact = joblib.load(MODEL_PATH)
    model = artifact['model']
    poly = artifact['poly']
    scaler = artifact['scaler']
    selector = artifact['selector']
    class_names = artifact['class_names']
    feature_names_original = artifact['features_used']
    print("✅ Model Loaded Successfully.")
except Exception as e:
    print(f"❌ Error Load Model: {e}")
    exit()

# Init MediaPipe (Drawing Utils DIHAPUS karena tidak dipakai lagi)
mp_face_mesh = mp.solutions.face_mesh
face_mesh = mp_face_mesh.FaceMesh(
    max_num_faces=1,
    refine_landmarks=True,
    min_detection_confidence=0.5,
    min_tracking_confidence=0.5
)

# ==========================================
# 2. OPTIMIZED MATH FUNCTIONS (PURE NUMPY)
# ==========================================
def extract_features_optimized(landmarks, w, h, poly_transformer, scaler_transformer, selector_transformer, feature_order):
    # Konversi Landmarks ke NumPy Array
    coords = np.array([(p.x * w, p.y * h) for p in landmarks], dtype=np.float32)

    def dist(idx1, idx2):
        return np.linalg.norm(coords[idx1] - coords[idx2])

    def angle(idx1, idx2, idx3):
        a = dist(idx2, idx3)
        b = dist(idx1, idx2)
        c = dist(idx1, idx3)
        if a * b == 0: return 0.0
        val = (a**2 + b**2 - c**2) / (2 * a * b)
        return np.degrees(np.arccos(np.clip(val, -1.0, 1.0)))

    # Hitung Fitur
    face_length = dist(10, 152)
    face_width = dist(234, 454)
    jaw_width = dist(58, 288)
    chin_width = dist(172, 397)
    forehead_width = dist(103, 332)

    if face_width == 0 or jaw_width == 0: return None

    face_oval_indices = [
        10, 338, 297, 332, 284, 251, 389, 356, 454, 323, 361, 288, 
        397, 365, 379, 378, 400, 377, 152, 148, 176, 149, 150, 136, 
        172, 58, 132, 93, 234, 127, 162, 21, 54, 103, 67, 109
    ]
    oval_points = coords[face_oval_indices].astype(np.int32)
    
    area = cv2.contourArea(oval_points)
    perimeter = cv2.arcLength(oval_points, True)
    if area == 0: return None
    
    hull = cv2.convexHull(oval_points)
    x, y, wr, hr = cv2.boundingRect(oval_points)
    
    raw_feats = {
        'circularity': (4 * np.pi * area) / (perimeter ** 2),
        'solidity': area / float(cv2.contourArea(hull)),
        'extent': area / float(wr * hr),
        'ratio_len_width': face_length / face_width,
        'ratio_jaw_cheek': jaw_width / face_width,
        'ratio_forehead_jaw': forehead_width / jaw_width,
        'ratio_chin_jaw': chin_width / jaw_width,
        'avg_jaw_angle': (angle(93, 58, 152) + angle(323, 288, 152)) / 2
    }

    try:
        input_values = [raw_feats[name] for name in feature_order]
    except KeyError:
        return None

    X_raw = np.array([input_values])
    X_poly = poly_transformer.transform(X_raw)
    X_scaled = scaler_transformer.transform(X_poly)
    X_final = selector_transformer.transform(X_scaled)
    
    return X_final

def analyze_skintone_simple(image, landmarks):
    h, w, _ = image.shape
    rois = [[330, 347, 346, 352], [101, 118, 117, 123]]
    mean_colors = []
    
    for roi in rois:
        points = np.array([[int(landmarks[i].x * w), int(landmarks[i].y * h)] for i in roi])
        mask = np.zeros((h, w), dtype=np.uint8)
        cv2.fillPoly(mask, [points], 255)
        mean_val = cv2.mean(image, mask=mask)[:3]
        mean_colors.append(mean_val)
    
    avg_bgr = np.mean(mean_colors, axis=0)
    lab = cv2.cvtColor(np.uint8([[avg_bgr]]), cv2.COLOR_BGR2LAB)[0][0]
    L, a, b = lab
    
    if L > 165: return "Fair Light"
    elif L > 105: return "Medium Tan"
    else: return "Deep"

# ==========================================
# 3. REALTIME LOGIC
# ==========================================
cap = cv2.VideoCapture(0)
cap.set(cv2.CAP_PROP_FPS, 30)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)

STATE_SCANNING = 0
STATE_RESULT_LIVE = 1
current_state = STATE_SCANNING

stable_frames = 0
REQUIRED_FRAMES = 5
final_shape = ""
final_tone = ""
process_time_ms = 0.0
prev_time = 0

print("\n🚀 Camera Started. Please look at the camera...")

# while cap.isOpened():
#     success, frame = cap.read()
#     if not success: break
    
#     frame = cv2.flip(frame, 1)
#     h, w, c = frame.shape
    
#     # FPS Counter
#     curr_time = time.time()
#     fps = 1 / (curr_time - prev_time) if (curr_time - prev_time) > 0 else 0
#     prev_time = curr_time

#     # --- STATE: SCANNING ---
#     if current_state == STATE_SCANNING:
#         rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
#         results = face_mesh.process(rgb_frame)
        
#         status_text = "Looking for face..."
#         color_status = (0, 0, 255)

#         if results.multi_face_landmarks:
#             landmarks_proto = results.multi_face_landmarks[0]
#             landmarks = landmarks_proto.landmark
            
#             # [HAPUS BAGIAN VISUALISASI MESH DISINI]
#             # Wajah sekarang bersih, tidak ada garis-garis.

#             # Cek Lurus
#             nose_x = landmarks[1].x
#             left_cheek_x = landmarks[234].x
#             right_cheek_x = landmarks[454].x
#             dist_L = nose_x - left_cheek_x
#             dist_R = right_cheek_x - nose_x
#             ratio = dist_L / dist_R if dist_R != 0 else 0
#             is_centered = 0.7 < ratio < 1.3
            
#             if is_centered:
#                 stable_frames += 1
#                 progress = min(stable_frames / REQUIRED_FRAMES, 1.0)
                
#                 # Visualisasi Bar
#                 bar_w = int(w * progress)
#                 cv2.rectangle(frame, (0, h-10), (bar_w, h), (0, 255, 0), -1)
#                 status_text = f"HOLD STILL... {int(progress*100)}%"
#                 color_status = (0, 255, 0)
                
#                 if stable_frames >= REQUIRED_FRAMES:
#                     print("📸 ANALYZING FAST...")
                    
#                     # [START TIMER] - Mulai hitung proses AI
#                     t_start = time.perf_counter()
                    
#                     X_ready = extract_features_optimized(
#                         landmarks, w, h, poly, scaler, selector, feature_names_original
#                     )
                    
#                     if X_ready is not None:
#                         pred_idx = model.predict(X_ready)[0]
#                         prob = np.max(model.predict_proba(X_ready))
#                         final_shape = f"{class_names[pred_idx]} ({prob*100:.0f}%)"
#                     else:
#                         final_shape = "Unknown"
                    
#                     final_tone = analyze_skintone_simple(frame, landmarks)
                    
#                     # [STOP TIMER] - Selesai hitung
#                     t_end = time.perf_counter()
#                     process_time_ms = (t_end - t_start) * 1000
                    
#                     print(f"✅ Result: {final_shape} | {final_tone}")
#                     print(f"⚡ Processing Time: {process_time_ms:.2f} ms")
                    
#                     current_state = STATE_RESULT_LIVE

#             else:
#                 stable_frames = 0
#                 status_text = "Please Look Straight"
#                 color_status = (0, 165, 255)
        
#         cv2.putText(frame, status_text, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, color_status, 2)

#     # --- STATE: RESULT ---
#     elif current_state == STATE_RESULT_LIVE:
#         # Background Overlay Hitam Transparan
#         overlay = frame.copy()
#         cv2.rectangle(overlay, (0, 0), (450, 180), (0, 0, 0), -1)
#         alpha = 0.6
#         frame = cv2.addWeighted(overlay, alpha, frame, 1 - alpha, 0)
        
#         cv2.putText(frame, "ANALYSIS COMPLETE", (20, 30), 
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
#         cv2.putText(frame, f"SHAPE: {final_shape}", (20, 80), 
#                     cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
#         cv2.putText(frame, f"SKIN : {final_tone}", (20, 125), 
#                     cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2)
        
#         # Tampilkan Speed
#         cv2.putText(frame, f"Speed: {process_time_ms:.2f} ms", (20, 165), 
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
        
#         cv2.putText(frame, "[Q] Quit  [R] Rescan", (20, h-30), 
#                     cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 200, 200), 2)

#     # Info FPS
#     cv2.putText(frame, f"FPS: {int(fps)}", (w - 120, 40), 
#                 cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

#     cv2.imshow('Smart Beauty Scan', frame)
    
#     key = cv2.waitKey(1) & 0xFF
#     if key == ord('q'):
#         break
#     elif key == ord('r'):
#         current_state = STATE_SCANNING
#         stable_frames = 0
#         print("🔄 Rescanning...")

# cap.release()
# cv2.destroyAllWindows()

# Tambahkan variabel ini sebelum loop 'while'
t_result_start = 0 

while cap.isOpened():
    success, frame = cap.read()
    if not success: break
    
    frame = cv2.flip(frame, 1)
    h, w, c = frame.shape
    
    # FPS Counter
    curr_time = time.time()
    fps = 1 / (curr_time - prev_time) if (curr_time - prev_time) > 0 else 0
    prev_time = curr_time

    # --- STATE: SCANNING ---
    if current_state == STATE_SCANNING:
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = face_mesh.process(rgb_frame)
        
        status_text = "Looking for face..."
        color_status = (0, 0, 255)

        if results.multi_face_landmarks:
            landmarks = results.multi_face_landmarks[0].landmark
            
            # Cek Wajah Lurus (Center)
            nose_x = landmarks[1].x
            left_cheek_x = landmarks[234].x
            right_cheek_x = landmarks[454].x
            dist_L = nose_x - left_cheek_x
            dist_R = right_cheek_x - nose_x
            ratio = dist_L / dist_R if dist_R != 0 else 0
            is_centered = 0.7 < ratio < 1.3
            
            if is_centered:
                stable_frames += 1
                progress = min(stable_frames / REQUIRED_FRAMES, 1.0)
                
                # Visualisasi Bar
                bar_w = int(w * progress)
                cv2.rectangle(frame, (0, h-10), (bar_w, h), (0, 255, 0), -1)
                status_text = f"ANALYZING... {int(progress*100)}%"
                color_status = (0, 255, 0)
                
                if stable_frames >= REQUIRED_FRAMES:
                    t_start_proc = time.perf_counter()
                    
                    # Ekstraksi Fitur & Prediksi
                    X_ready = extract_features_optimized(landmarks, w, h, poly, scaler, selector, feature_names_original)
                    
                    if X_ready is not None:
                        pred_idx = model.predict(X_ready)[0]
                        prob = np.max(model.predict_proba(X_ready))
                        final_shape = f"{class_names[pred_idx]} ({prob*100:.0f}%)"
                    
                    final_tone = analyze_skintone_simple(frame, landmarks)
                    process_time_ms = (time.perf_counter() - t_start_proc) * 1000
                    
                    # PINDAH KE STATE RESULT & CATAT WAKTUNYA
                    current_state = STATE_RESULT_LIVE
                    t_result_start = time.time() 

            else:
                stable_frames = 0
                status_text = "Please Look Straight"
                color_status = (0, 165, 255)
        
        cv2.putText(frame, status_text, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, color_status, 2)

    # --- STATE: RESULT (AUTO-RESCAN LOGIC) ---
    elif current_state == STATE_RESULT_LIVE:
        # Overlay Hitam Transparan
        overlay = frame.copy()
        cv2.rectangle(overlay, (0, 0), (450, 180), (0, 0, 0), -1)
        frame = cv2.addWeighted(overlay, 0.6, frame, 0.4, 0)
        
        cv2.putText(frame, "ANALYSIS COMPLETE", (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
        cv2.putText(frame, f"SHAPE: {final_shape}", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
        cv2.putText(frame, f"SKIN : {final_tone}", (20, 125), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2)
        
        # Hitung waktu mundur (Countdown)
        elapsed = time.time() - t_result_start
        remaining = max(0, 2.0 - elapsed)
        
        cv2.putText(frame, f"Auto-refresh in: {remaining:.1f}s", (20, 165), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)

        # TRIGGER AUTO-RESCAN JIKA SUDAH 2 DETIK
        if elapsed >= 1.5:
            current_state = STATE_SCANNING
            stable_frames = 0

    # Info FPS
    cv2.putText(frame, f"FPS: {int(fps)}", (w - 120, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.imshow('Smart Beauty Scan', frame)
    
    if cv2.waitKey(1) & 0xFF == ord('q'): break

cap.release()
cv2.destroyAllWindows()